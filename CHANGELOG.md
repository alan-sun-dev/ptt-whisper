...
`ptt_whisper.lua` 未變動，僅同步版本號。實際改動在 `transcribe.sh`
2.12.0 → **2.13.0**。

### Added — whisper 的標點有固定偏差

掃過 39 個快取檔（954 字，都是真實口述內容）：
...
**逗號和問號 100% 是半形，句號 100% 是全形，半形句點出現 0 次。**

中文行文裡夾 ASCII 的 `,` 和 `?` 是排版錯誤，而且它**影響每一句話**——
不像術語錯誤只影響偶爾提到的幾個詞。
...
**初版只檢查「前一個字是漢字」，那是錯的。** 第三方 review（Codex）指出
那個判別式看的是**文字系統**、不是**語言脈絡**，實測會改壞中英混雜的內容：
...
逗號前面確實是漢字「名」，但那個逗號屬於 SQL 語法，不是中文標點。

改成**前後都要是漢字**之後，這三個都不再被動到，而語料裡 40 個該轉的仍全中。
句末的 `?` `!` 另外處理（後面是漢字**或字串結尾**）——但 `,` `;` `:`
**不放寬到結尾**：中文散文不會用逗號結尾，出現在結尾的逗號分號比較可能是
程式碼，`SELECT 值;` 正是這樣被擋下來的。

這條規則天然保護了 `4.1.2`、`12:30`、`HT-H7608` 這類數字與識別碼。

**誤轉的代價遠高於漏轉。** 所以「vLLM, 效果不錯」這種前面是英文的逗號**不會**
被轉：中文排版上它其實該轉，但資料裡出現 0 次，為此放寬規則不划算。

**半形句點一律不碰**：它與小數點、版本號、縮寫難以區分，而 whisper 本來就
輸出全形「。」——資料裡半形句點 0 次，改它是徒增風險沒有收益。

### 只涵蓋中文，日文與韓文刻意不處理

`これは?` 不會被轉。review 把這列為 bug，**但放寬到假名才是錯的**：
日文的逗號是「、」(U+3001) 而不是「，」(U+FF0C)，放寬規則會產生排版錯誤
的日文——比不轉更糟。

處理方式是**把限制釘住**：程式碼註解寫明只涵蓋中文，並加上日文假名與韓文
諺文的斷言，防止日後有人「順手」放寬判別式。要支援日文需要另一套對應表，
那是獨立功能。

### 必須是 `\p{Script=Han}`，不是 `\p{Han}`

寫測試時撞到的，**而且 review 這一項判斷錯了**（它標為 UNFOUNDED，但它自己
貼出的探測輸出其實顯示有轉）：
...
perl 的 `\p{Han}` 預設是 **Script_Extensions**，會把「。」「、」這類漢字圈
**共用標點**也算成 Han。改用嚴格的 `\p{Script=Han}` 才是真正的「漢字」。
已加斷言釘住。

### 實作細節

用 `perl` 而非 `sed`：本腳本全域 `export LC_ALL=C`，sed 在該 locale 下處理
多位元組不可靠；而 perl 的 Unicode script 屬性直接表達「漢字」這個意圖，
比手刻 UTF-8 位元組範圍好審閱得多。對應表用 `\x{FF0C}` 這類 codepoint
escape，避免腳本自身的編碼問題。

`/usr/bin/perl` 是 macOS 內建，但仍照本專案慣例做成**失敗即跳過**，
不成為硬依賴。**轉換失敗一律保留原文**——寧可標點沒轉，也不能把整句話弄不見。

### Testing

`./tests/run.sh` → **277 passed, 0 failed, 0 skipped**（前一版 253）。

新增 `tests/cases/55-punctuation.sh`（24 條）。重點不在「有沒有轉」，
而在**「不該轉的一個都沒動」**——過半是負向斷言。

**雙向驗證**：把規則放寬成無條件轉換，那些「不該轉」的斷言確實全部變紅。

review 之後補上的斷言：

- SQL 中文欄位名後的逗號、中文接英文的冒號、結尾分號、分號後接空白
- 日文假名與韓文諺文（**釘住「只做中文」這個決定**）
- 全形句號後的半形逗號（**釘住 `Script=Han` 而非 `Han`**）
- 快取命中的行為（review 正確指出這條沒被覆蓋）

降級路徑也測到了，而且是**可達的**失敗：餵進非法 UTF-8 位元組時 perl 會噴
Malformed UTF-8 且 **stdout 為空**，正好觸發 guard。斷言確認文字沒有消失、
log 有記錄降級。
（「perl 完全不存在」那條測不到——查找是絕對路徑優先，PATH 蓋不掉，
與 `find_ffmpeg` 同樣的情況。）

### 已知的小落差

**既有的快取條目不會被回頭轉換。** 快取命中會直接輸出並結束，不經過文字
管線。目前 24 筆舊條目在被 LRU 汰換前，重播時仍是舊的半形標點。
不併入 variant hash——這是管線步驟不是參數，把它當參數會誤導。
新版寫入的條目本身已經是轉換後的，所以命中也是正確的（已有斷言覆蓋）。

**規則是從 954 字、單一使用者的語料推導出來的。** review 正確指出這個樣本
不足以支撐一個通用的語言脈絡判別式。目前的處置是**把規則收到夠緊**——
寧可漏轉也不誤轉——而不是假裝樣本足夠。日後若出現漏轉的實例，
應該拿新的實例來放寬，而不是憑想像。

### 未做（一併記錄判斷依據）

**OpenCC `s2twp` 暫不實作。** 同一份資料：
...
`initial_prompt` 已經把簡體字的問題解決了。為 2 處用詞引入新依賴
（`opencc` 這台機器沒裝）＋新管線階段＋新失敗路徑不划算。
**觸發條件：哪天輸出開始出現簡體字再做**（例如改了 prompt 或換模型）。
